...
from tartiflette.validation.rules import ValidObjectDefinitionsRule
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                TartifletteError(
                    message="Object type < Foo > can only include field fieldA once.",
                    locations=[
                ...
                TartifletteError(
                    message="Object type < Foo > can only include field fieldB once.",
                    locations=[
...
async def test_valid_object_definitions(sdl, expected):
    assert (
        validate_sdl(
            parse_to_document(sdl), rules=[ValidObjectDefinitionsRule]
        )
        == expected
    )
