...
__all__ = ("ValidInterfaceDefinitionsRule",)
...
class ValidInterfaceDefinitionsRule(ASTValidationRule):
    """
    TODO:
    """
    ...
    def enter_InterfaceTypeDefinition(
        self, node: "InterfaceTypeDefinitionNode", *_
    ):
        """
        TODO:
        :param node: TODO:
        ...
        if not _is_valid_name(interface_name):
            self.context.report_error(
                graphql_error_from_nodes(
                    f'Name < {interface_name} > must not begin with "__", '
        ...
        if node.fields:
            for field_definition in node.fields:
                ...
                if known_field:
                    self.context.report_error(
                        graphql_error_from_nodes(
                            f"Interface type < {interface_name} > can only "
                ...
                if not isinstance(wrapped_field_type, _OUTPUT_TYPES):
                    self.context.report_error(
                        graphql_error_from_nodes(
                            "The type of "
                            f"< {interface_name}.{field_name} > must be "
                    ...
                    for argument_definition in field_definition.arguments:
                        ...
                        if known_argument:
                            self.context.report_error(
                                graphql_error_from_nodes(
                                    f"Field argument "
                                    f"< {interface_name}.{field_name}({argument_name}) > "
